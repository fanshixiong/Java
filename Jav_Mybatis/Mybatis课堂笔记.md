...
- 接口实现类由原来的UserDaoImpl转变为一个 Mapper配置文件.
  ...
  <!--select查询语句-->
     <select id="getUserList" resultType="com.frans.pojo.User">
...
org.apache.ibatis.binding.BindingException: Type interface com.frans.dao.UserDao is not known to the MapperRegistry.
...
2. 编写对应的mapper中的sql语句
   ...
   ```java
   <select id="getUserById" parameterType="int" resultType="com.frans.pojo.User">
...
```xml
    <!--对象中的属性，可以直接取出来-->
    <insert id="addUser" parameterType="com.frans.pojo.User">
...
```xml
    <update id="updateUser" parameterType="com.frans.pojo.User">
...
```xml
    <!--可以给实体类起别名-->
    <typeAliases>
        <typeAlias type="com.frans.pojo.User" alias="User"/>
    </typeAliases>
```
...
<!--每一个Mapper.XML都需要在Mybatis核心配置文件中注册！-->
<mappers>
    <mapper class="com.frans.dao.UserMapper"/>
</mappers>
```
...
>若用方式三，需要在resources相同的包，编译后class文件和xml文件就在同一包下。[JVM张龙](https://www.bilibili.com/video/BV187411B7iv?from=search&seid=17485209067813213236)
...
- 起别名
  ...
  ```xml
  <select id="getUserById" resultType="com.frans.pojo.User">
...
2. log4j.properties
   ...
   log4j.appender.file.File=./log/frans.log
...
3. 测试
   ...
   //通过Java代码层面实现分页
   List<User> userList = sqlSession.selectList("com.frans.dao.UserMapper.getUserByRowBounds",null,rowBounds);
...
3. 测试
   ...
   1. 问题:我们需要将实体类序列化！否则就会报错！
      ...
      ```
      Caused by: java.io.NotSerializableException: com.frans.pojo.User
      ```
